...
void FSharpHotReloadClassReinstancer::CollectChangedProperties(FCDOPropertyContainer& Container, TSet<UObject*>& SeenObjects)
{
	...
	{
		for (TFieldIterator<FProperty> PropertyIt(Container.OldStruct); PropertyIt; ++PropertyIt)
		{
	...
	else
	{
		...
		for (TFieldIterator<FProperty> PropertyIt(Container.NewStruct); PropertyIt; ++PropertyIt)
		{
			FProperty* Property = *PropertyIt;
			FCDOPropertyInfo& PropertyInfo = Properties.FindOrAdd(Property->GetFName());
			PropertyInfo.NewProperty = Property;
	...
	for (const TPair<FName, FCDOPropertyInfo>& Pair : Properties)
	{
		...
		FProperty* OldProperty = Property.OldProperty;
		FProperty* NewProperty = Property.NewProperty;
		if (OldProperty == nullptr || NewProperty == nullptr)
		{
			continue;
		}

		FFieldClass* OldPropertyClass = OldProperty->GetClass();
		FFieldClass* NewPropertyClass = NewProperty->GetClass();

		const EClassCastFlags BaseCastFlags = CASTCLASS_UField | CASTCLASS_FProperty;

		if (OldPropertyClass->GetCastFlags() == NewPropertyClass->GetCastFlags())
		{
			switch ((uint64)OldPropertyClass->GetCastFlags())
			{
				// Primitive types
				case (BaseCastFlags | CASTCLASS_FNumericProperty | CASTCLASS_FInt8Property):
				case (BaseCastFlags | CASTCLASS_FNumericProperty | CASTCLASS_FByteProperty):
				case (BaseCastFlags | CASTCLASS_FNumericProperty | CASTCLASS_FIntProperty):
				case (BaseCastFlags | CASTCLASS_FNumericProperty | CASTCLASS_FFloatProperty):
				case (BaseCastFlags | CASTCLASS_FNumericProperty | CASTCLASS_FUInt64Property):
				case (BaseCastFlags | CASTCLASS_FNumericProperty | CASTCLASS_FUInt32Property):
				case (BaseCastFlags | CASTCLASS_FBoolProperty):
				case (BaseCastFlags | CASTCLASS_FNumericProperty | CASTCLASS_FUInt16Property):
				case (BaseCastFlags | CASTCLASS_FNumericProperty | CASTCLASS_FInt64Property):
				case (BaseCastFlags | CASTCLASS_FNumericProperty | CASTCLASS_FInt16Property):
				case (BaseCastFlags | CASTCLASS_FNumericProperty | CASTCLASS_FDoubleProperty):
				case (BaseCastFlags | CASTCLASS_FEnumProperty):
				// Name / string
				case (BaseCastFlags | CASTCLASS_FNameProperty):
				case (BaseCastFlags | CASTCLASS_FStrProperty):
				// Delegates
				case (BaseCastFlags | CASTCLASS_FDelegateProperty):
				case (BaseCastFlags | CASTCLASS_FMulticastDelegateProperty):
				case (BaseCastFlags | CASTCLASS_FMulticastDelegateProperty | CASTCLASS_FMulticastSparseDelegateProperty):
					{
					...
					break;
				// Text
				case (BaseCastFlags | CASTCLASS_FTextProperty):
					{
						// FTextProperty::Identical / FText::IdenticalTo will often return true for differing values
						// (TextData->GetLocalizedString() will often return "Null" on differing strings whose values are in History)
						FTextProperty* OldTextProperty = CastFieldChecked<FTextProperty>(OldProperty);
						FTextProperty* NewTextProperty = CastFieldChecked<FTextProperty>(NewProperty);
					...
					break;
				// Collections
				case (BaseCastFlags | CASTCLASS_FSetProperty):
				case (BaseCastFlags | CASTCLASS_FMapProperty):
				case (BaseCastFlags | CASTCLASS_FArrayProperty):
					break;
				// Interface
				case (BaseCastFlags | CASTCLASS_FInterfaceProperty):
					break;
				// FObjectPropertyBase types
				case (BaseCastFlags | CASTCLASS_FObjectPropertyBase | CASTCLASS_FObjectProperty | CASTCLASS_FClassProperty):
				case (BaseCastFlags | CASTCLASS_FObjectPropertyBase | CASTCLASS_FWeakObjectProperty):
				case (BaseCastFlags | CASTCLASS_FObjectPropertyBase | CASTCLASS_FLazyObjectProperty):
				case (BaseCastFlags | CASTCLASS_FObjectPropertyBase | CASTCLASS_FSoftObjectProperty):
				case (BaseCastFlags | CASTCLASS_FObjectPropertyBase | CASTCLASS_FSoftClassProperty):
					break;
				case (BaseCastFlags | CASTCLASS_FObjectPropertyBase | CASTCLASS_FObjectProperty):
					{
						FObjectProperty* OldObjectProperty = CastFieldChecked<FObjectProperty>(OldProperty);
						FObjectProperty* NewObjectProperty = CastFieldChecked<FObjectProperty>(NewProperty);
					...
					break;
				case (BaseCastFlags | CASTCLASS_FStructProperty):
					{
						...
						StructContainer.OldStruct = CastField<FStructProperty>(OldProperty)->Struct;
						StructContainer.NewStruct = CastField<FStructProperty>(NewProperty)->Struct;
				...
				default:
					//check(0);
					UE_LOG(LogTemp, Fatal, TEXT("Unhandled property class cast flags %llu"), (uint64)OldPropertyClass->GetCastFlags());
					break;
			}
		}
		else
...
void FSharpHotReloadClassReinstancer::UpdateDefaultProperties(const FCDOPropertyContainer& Container, void* Obj)
{
	...
	for (const FCDOPropertyContainer& ChildContainer : Container.Children)
	{
		...
		const EClassCastFlags BaseCastFlags = CASTCLASS_UField | CASTCLASS_FProperty;
		switch ((uint64)ChildContainer.NewProperty->GetClass()->GetCastFlags())
		{
			case (BaseCastFlags | CASTCLASS_FObjectPropertyBase | CASTCLASS_FObjectProperty):
				{
					FObjectProperty* ObjectProperty = CastFieldChecked<FObjectProperty>(ChildContainer.NewProperty);
