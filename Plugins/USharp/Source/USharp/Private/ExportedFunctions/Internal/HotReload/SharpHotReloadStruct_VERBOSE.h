...
struct FUserDefinedStructureCompilerInner
{
		...
		TArray<UUserDefinedStruct*>& ChangedStructs)
	{
		if (StructureToReinstance)
		{
			...
			for (auto StructProperty : TObjectRange<FStructProperty>(RF_ClassDefaultObject, /** bIncludeDerivedClasses */ true, /** InternalExcludeFlags */ EInternalObjectFlags::Garbage))
			{
			...
			for (auto Blueprint : TObjectRange<UBlueprint>(RF_ClassDefaultObject, /** bIncludeDerivedClasses */ true, /** InternalExcludeFlags */ EInternalObjectFlags::Garbage))
			{
	...
	static UObject* CleanAndSanitizeStruct(UUserDefinedStruct* StructToClean)
	{
		...
		if (FStructureEditorUtils::FStructEditorManager::ActiveChange != FStructureEditorUtils::EStructureEditorChangeInfo::DefaultValueChanged)
		{
			...
			for (auto SubObjIt = SubObjects.CreateIterator(); SubObjIt; ++SubObjIt)
			{
				...
				if (FProperty* Prop = Cast<FProperty>(CurrSubObj))
				{
...
void SharpHotReloadUtils_PostUpdateStructs(TArray<UScriptStruct*>& SharpChangedStructs, TSet<UBlueprint*>* InBlueprintsToRecompile, TArray<UUserDefinedStruct*>* InChangedStructs)
{
	...
	for (TObjectIterator<UK2Node> It(RF_Transient | RF_ClassDefaultObject, /** bIncludeDerivedClasses */ true, /** InternalExcludeFlags */ EInternalObjectFlags::Garbage); It && SharpChangedStructs.Num(); ++It)
	{
		bool bReconstruct = false;

		UK2Node* Node = *It;

		if (Node && !Node->HasAnyFlags(RF_Transient) && IsValid(Node))
		{
